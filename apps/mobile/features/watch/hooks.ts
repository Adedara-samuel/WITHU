...
export function useWatchSession(id: string | undefined) {
  return useQuery({ queryKey: ["watch-session", id], queryFn: () => watchApi.getSession(id!), enabled: !!id });
}
...
export function useWatchRealtime(sessionId: string | undefined) {
  ...
  useEffect(() => {
    ...
    const onUpdated = ({ session }: { session: WatchSession }) => queryClient.setQueryData(["watch-session", session.id], session);
    const onEnded = () =>
