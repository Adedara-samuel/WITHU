...
// two devices behind restrictive/symmetric NATs may fail to connect peer-to-peer.
...
function toPlainCandidate(c: RTCIceCandidate): PlainIceCandidate {
...
export function CallProvider({ children }: { children: React.ReactNode }) {
  ...
  const createPeerConnection = useCallback(
    (callId: string) => {
      ...
      pc.onicecandidate = (event) => {
        if (event.candidate) {
          socket.emit("CALL_SIGNAL", { callId, signal: { type: "ice-candidate", candidate: toPlainCandidate(event.candidate) } });
        }
      };
      pc.ontrack = (event) => {
        setState((prev) => (prev.callId === callId ? { ...prev, phase: "active", remoteStream: event.streams[0] ?? null, startedAt: prev.startedAt ?? Date.now() } : prev));
      };
  ...
  const respondToCall = useCallback(
    async (accept: boolean) => {
      ...
      try {
        const localStream = await navigator.mediaDevices.getUserMedia({ audio: true, video: session.kind === "video" });
  ...
  useEffect(() => {
    ...
    const onSignal = async ({ callId, signal }: { callId: string; signal: { type: string; sdp?: string; candidate?: PlainIceCandidate } }) => {
      ...
      if (signal.type === "offer" && signal.sdp) {
        await session.pc.setRemoteDescription({ type: "offer", sdp: signal.sdp });
